import { z } from 'zod';
import { ArchetypeName, ArchetypeParams } from './archetypes.js';
 ...
 */
export function parseVideoSpec(input) {
    ...
    for (const [index, beat] of spec.beats.entries()) {
        ...
        if (beat.archetype === 'fan_out') {
            ...
            if (p.highlightIndex !== undefined && p.highlightIndex >= p.targets.length) {
                issues.push(`beats.${index} (fan_out) params.highlightIndex: ${p.highlightIndex} is out of range for ${p.targets.length} targets`);
            }
        }
        if (beat.archetype === 'spatial_map') {
            ...
            for (const [edgeIndex, edge] of p.edges.entries()) {
                if (edge.from >= p.nodes.length || edge.to >= p.nodes.length) {
                    issues.push(`beats.${index} (spatial_map) params.edges.${edgeIndex}: references a node index that does not exist`);
                }
                if (edge.from === edge.to) {
                    issues.push(`beats.${index} (spatial_map) params.edges.${edgeIndex}: an edge cannot start and end at the same node`);
                }
            }
        }
        if (beat.archetype === 'accumulation') {
            const p = params.data;
            const monotonic = p.stages.every((stage, i) => i === 0 || stage.magnitude >= p.stages[i - 1].magnitude);
            if (!monotonic) {
                issues.push(`beats.${index} (accumulation) params.stages: magnitudes must not decrease — this archetype animates growth`);
            }
        }
        beats.push({ ...beat, params: params.data });
    }
    ...
    for (const [index, beat] of spec.beats.entries()) {
        ...
        if (words > maxWords) {
            issues.push(`beats.${index}.narration: ${words} words will not fit in ${beat.durationSeconds}s (max ~${maxWords}). Shorten it or lengthen the beat.`);
        }
        for (const phrase of beat.emphasis) {
            if (!beat.narration.toLowerCase().includes(phrase.toLowerCase())) {
                issues.push(`beats.${index}.emphasis: "${phrase}" does not appear in this beat's narration, so it can never be synced`);
            }
        }
    }
    ...
    if (drift > spec.totalDurationTarget * 0.25) {
        issues.push(`beats: durations plus holds total ${total.toFixed(1)}s but totalDurationTarget is ${spec.totalDurationTarget}s — bring them within 25%`);
    }
    if (issues.length > 0) {
